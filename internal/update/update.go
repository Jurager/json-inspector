// Package update implements self-update for the application. Releases are
// published as GitHub Releases, each carrying platform archives plus a
// checksums.txt. The macOS archive is a code-signed .app bundle; updating swaps
// the whole bundle (never the binary inside it) so the signature stays intact.
package update
...
// Current holds the running version. main sets it from the build-time value
...
// assetName returns the release archive for this OS/arch:
// json-inspector-darwin-arm64.app.zip, …-windows-amd64.exe.zip, …-linux-amd64.tar.gz.
...
// fetchRelease loads a release by its subpath: "latest" or "tags/<version>".
...
// StartupCheck is the background check run at launch. It is throttled to once
// a day, skipped for dev builds, and returns nil when there is nothing new.
...
// Apply downloads, verifies and installs version, then relaunches the app.
// On success this call does not return (the process is replaced or exits).
...
// fetchChecksum downloads checksums.txt (a release asset) and returns the
// sha256 of the named asset.
...
// parseVersion splits "v1.2.3" into numeric components. A build-metadata or
// pre-release suffix is dropped so a local "v1.0.6-dirty" compares as 1.0.6.
