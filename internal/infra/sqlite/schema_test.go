...
// This file is the only place that runs the real embedded schema instead of a fixture. It is what
// proves the .sql files shipped inside the binary actually parse, run in order, and leave behind
// the schema the rest of the app queries by name.
//
// It runs here rather than beside the migration runner because it tests none of the runner: every
// assertion below is about our tables, our cascades and our pragmas. The runner knows nothing of
// them — which is also why it can live in pkg/ and be lifted whole into another project.
...
func TestEmbeddedSchemaApplies(t *testing.T) {
	...
	result, err := store.Migrate(ctx, migrations.FS)
	if err != nil {
	...
	again, err := store.Migrate(ctx, migrations.FS)
	if err != nil {
		t.Fatalf("second Migrate(migrations.FS): %v", err)
	}
...
// much later as missing data. So the path is asserted end to end, on the connection the app really
// opens: the pragma reports 1, a dangling child row is refused, a delete really cascades.
func TestEmbeddedSchemaEnforcesForeignKeys(t *testing.T) {
	...
	_, err := store.DB().ExecContext(ctx,
		`INSERT INTO record_bodies (record_seq, side) VALUES (?, ?)`, 999999, "request")
	if err == nil {
	...
	if _, err := store.DB().ExecContext(ctx, `DELETE FROM records WHERE id = ?`, "rec-1"); err != nil {
		t.Fatalf("deleting the record: %v", err)
	}

	var bodies int
	if err := store.DB().QueryRow(`SELECT count(*) FROM record_bodies`).Scan(&bodies); err != nil {
...
func TestEmbeddedSchemaCascadesAWorkspace(t *testing.T) {
	...
	for _, s := range seed {
		if _, err := store.DB().ExecContext(ctx, s.sql, s.args...); err != nil {
			t.Fatalf("inserting %s: %v", s.what, err)
		}
	}

	if _, err := store.DB().ExecContext(ctx, `DELETE FROM workspaces WHERE id = ?`, ws); err != nil {
	...
	if err := store.DB().QueryRow(personal).Scan(&kept); err != nil {
