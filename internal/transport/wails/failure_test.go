...
// What the window reads off a rejected call. The code and the args have to be there; the sentinel, the
// English text and Go's own fields must not.
...
// A site that wraps a bare sentinel says nothing, and the window still learns what kind of refusal it
// was — that is what keeps most call sites out of this work.
...
// Nothing comes back for an error the app did not build: the option then falls through to Wails' own
// marshaller, and the window shows the machine's message rather than a sentence nobody wrote.
