...
* тема — класс `dark`/`light` на `<html>`: палитры в `style.css` (`:root` и `:root.dark`), значение в `localStorage` под `ji-theme-v1`, ставит `composables/useTheme.ts`. Смена темы идёт растущим кругом из левого верхнего угла — заливка приходит по диагонали и достаёт правый нижний угол последним (view transition, `@keyframes theme-wipe` в `style.css`); там, где API нет, — мгновенно. Пилюля переключателя доезжает до нового сегмента раньше смены темы (свой `transition` в `ThemeSwitch.vue`): внутри перехода окно заморожено снимком, и пилюля, пойманная на полпути, застыла бы там на всю заливку, а её собственный слой накрыл бы иконку, к которой она едет. Новые тёмные правила писать только в `:root.dark` — медиазапроса `prefers-color-scheme` там больше нет. Те же правила продублированы инлайновым скриптом в `index.html` и `about.html`: он ставит класс до первой отрисовки, чтобы тёмное окно не мигало светлым, и меняется вместе с композаблом.
* `composables/` — повторяющаяся реактивная логика (`useToast`, `useResizableWidth`) и модульные синглтоны без реактивности (`urlFocus`: поле командной строки живёт меньше, чем запрос фокуса к нему).
