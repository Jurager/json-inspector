...
**дерево коллекций и прогон** (`usecase/collection`: узлы, дублирование поддерева, обход и
последовательная отправка; импорт и экспорт Postman — дальше по M5). Наследование авторизации
...
Ещё живёт в TS и переезжает дальше: скрипты (M6) и Postman-импорт/экспорт с сохранением запроса
из командной строки (M5.3, M5.5). Из `lib/` остались порты, которые
