...
текстом), **история** (`usecase/record`: отправка, запись, тела, ретенция, импорт легаси) и
...
`EnvironmentsService`, `BridgeService`; окна и меню живут в `transport/wails`, мост расширения — в
`transport/bridge`.
...
Ещё живёт в TS и переезжает дальше: коллекции (M5), скрипты (M6). Из `lib/` остались порты, которые
