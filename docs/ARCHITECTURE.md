...
на goja). Наследование авторизации описано в схеме (`NULL` = «возьми у родителя»), но пока не
применяется: `prepare` черновика разрешает токен Auth и не подставляет его в запрос — это отдельное
решение, и оно меняет поведение командной строки тоже.
