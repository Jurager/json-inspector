...
собственные. Отчёт каждого прогона скрипта висит на записи, вокруг которой он выполнялся, — поэтому
обе половины скриптов пишутся уже после отправки, а отчёты первой половины доезжают до неё в
`ScriptPass.Ran`. Чего в песочнице нет намеренно: сети, файлов, таймеров и `pm.sendRequest`.
