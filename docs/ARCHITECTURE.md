...
**черновик** (`usecase/draft`: модель, преобразования адреса и банки, превью, персистентность) и
**дерево коллекций, прогон и Postman** (`usecase/collection`: узлы, дублирование поддерева, обход
и последовательная отправка; `internal/postman` — импорт и экспорт). Наследование авторизации
описано в схеме (`NULL` = «возьми у родителя»), но пока не применяется: `prepare` черновика
разрешает токен Auth и не подставляет его в запрос — это отдельное решение, и оно меняет
поведение командной строки тоже.
Привязанные сервисы — `SystemService`, `SettingsService`, `RecordsService`, `DraftService`,
`EnvironmentsService`, `CollectionsService`, `BridgeService`; окна и меню живут в `transport/wails`,
мост расширения — в `transport/bridge`.
...
Ещё живёт в TS и переезжает дальше: скрипты (M6). Из `lib/` остались порты, которые
