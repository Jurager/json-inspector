...
import { RecordSource, RowKind, type CookieRow } from '../../../bindings/json-inspector/internal/domain'
...
// The answer's own cookies, read in Go out of the record's `Set-Cookie` rows.
const set = computed(() => props.record.responseCookies)

// The jar below them is the request's, and only a request this app composed has one: a captured
// record brought no jar, and the command line's is not its to show.
const isManual = computed(() => props.record.source === RecordSource.SourceManual)

// A path is worth a column only when it is not the one every cookie is given by default: `/` on
...
<template>
  <div class="req-cookies">
    <!-- What the answer said. Drawn above the jar because it is the half a person opened this tab
         for after a send: the request side is the one they typed. -->
    <template v-if="set.length > 0">
      <I18nT keypath="response.cookies.setIntro" tag="p" class="intro">
        <template #word><span class="code">{{ t('response.cookies.setHeader') }}</span></template>
      </I18nT>
      ...
      <div class="card">
        ...
        <div v-for="(c, i) in set" :key="i" class="req-cookies-row">
          <div class="cell-text mono">{{ c.name }}</div>
          <div class="cell-text mono value">{{ c.value }}</div>
          <div class="cell-text">
            {{ c.domain }}<span v-if="pathOf(c.path)" class="path">{{ pathOf(c.path) }}</span>
          </div>
          <div class="cell-text">{{ c.expires || t('response.cookies.session') }}</div>
          <div class="req-cookies-flags">
            <span class="flag" :class="{ on: c.secure }">Secure</span>
            <span class="flag" :class="{ on: c.httpOnly }">HttpOnly</span>
          </div>
          <div></div>
        </div>
      </div>
    ...
    <!-- What the request carries. Only a request this app made has a jar; the word the cookies
         travel under is drawn as the chip the window draws every other literal in. -->
      ...
      <div class="card">
        ...
        <div v-for="c in store.cookies" :key="c.id" class="req-cookies-row">
          ...
          <input
            ...
            :placeholder="t('response.cookies.session')"
            spellcheck="false"
            @input="patch(c, { expires: ($event.target as HTMLInputElement).value })"
          />
          <div class="req-cookies-flags">
            <button class="flag-btn" :class="{ active: c.secure }" @click="toggleFlag(c, 'secure')">
              Secure
            </button>
            <button class="flag-btn" :class="{ active: c.httpOnly }" @click="toggleFlag(c, 'httpOnly')">
              HttpOnly
            </button>
          </div>
...
/* A cell of the answer's table, where nothing is editable: the same padding the inputs stand on, so
   the two tables put their columns in the same places. */
.cell-text {
  @apply min-w-0 text-[13px] text-text-secondary overflow-hidden text-ellipsis whitespace-nowrap;
  padding: 0 16px;
}
...
/* A path that is not the default every cookie is given, kept at the lesser ink beside the domain it
   belongs to — it is the second half of "which cookie this is", not a column of its own. */
.cell-text .path {
  @apply ml-1.5 text-text-tertiary;
}
...
/* A flag is a state, not a control with a colour of its own: on is the window's "yes", off is the
   chip the row would otherwise be written on. A row of the answer's table states its flags rather
   than offering them, so it gets the same chip without the cursor. */
.flag,
.flag-btn {
  @apply text-[12px] font-semibold py-[3px] px-2 rounded-md border-none text-text-tertiary;
  background: var(--bg-hover);
